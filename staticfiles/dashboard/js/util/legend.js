// Retrieved from https://observablehq.com/@d3/color-legend
...
export function swatches({
    ...
    if (columns !== null) return `<div style="display: flex; align-items: center; margin-left: ${+marginLeft}px; min-height: 33px; font: 14px sans-serif;">
        ...
        <div style="width: 100%; columns: ${columns};">
            ...
            ${color.domain().map(value => {
                ...
                return `<div class="${id}-item ${className}" label="${label.replace(/["&]/g, entity)}">
    ...
    return `<div style="display: flex; align-items: center; min-height: 33px; margin-left: ${+marginLeft}px; font: 14px sans-serif;">
        ...
        <div>
            ${title !== "" ? `<div>${title}</div>` : ""}
            ${color.domain().map(value => `<span class="${id} ${className}" style="--color: ${color(value)}" label="${format(value).replace(/["&]/g, entity)}">${format(value)}</span>`)}
        </div>`;
}
